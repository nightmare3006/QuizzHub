...
export const QuizSolutions = () => {
    const location = useLocation();
    const navigate = useNavigate();
    const query = useMemo(() => new URLSearchParams(location.search), [location.search]);
    const { quizId } = useParams()
    const page = query.get("page") || 1;
    const [url, setUrl] = useState(`http://127.0.0.1:8000/quizhub/quiz/${quizId}/solutions/?page=${page}`);
    ...
    const handlePageChange = (newUrl) => {
        ...
        if (newPage) {
            ...
            navigate(`?page=1`);
            setUrl(`http://127.0.0.1:8000/quizhub/quiz/${quizId}/solutions/?page=1`);
        }
    };

    useEffect(() => {
        const newPage = query.get("page") || 1;
        setUrl(`http://127.0.0.1:8000/quizhub/quiz/${quizId}/solutions/?page=${newPage}&_=${new Date().getTime()}`);
